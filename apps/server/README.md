...
![ccweb 主界面](https://raw.githubusercontent.com/TimeCraker/ccweb/main/docs/screenshots/main-zh-dark.png)
...
| ⚙️ | **全图形设置** | 模型热切换、effort、权限模式、**上下文隔离**(全量/仅项目/纯净)、API 端点模板(凭证不出本地)、Skills / Rules / CLAUDE.md 查看 |
| 🌓 | **深浅双主题 · 中英双语 UI** | 断线自动重连对账(seq 重放,不丢消息) |
| 💾 | **一键导出 Markdown** | 完整对话含工具调用 |

## 架构

![architecture](https://raw.githubusercontent.com/TimeCraker/ccweb/main/docs/architecture.svg)
...
| | |
|---|---|
| ![审批卡](https://raw.githubusercontent.com/TimeCraker/ccweb/main/docs/screenshots/approval.png) | ![工具卡](https://raw.githubusercontent.com/TimeCraker/ccweb/main/docs/screenshots/toolcard.png) |
| Edit 审批:行级 diff 预览 + 允许/拒绝/总是允许 | Read 工具卡:参数 + 带行号结果 |
| ![命令面板](https://raw.githubusercontent.com/TimeCraker/ccweb/main/docs/screenshots/palette.png) | ![设置](https://raw.githubusercontent.com/TimeCraker/ccweb/main/docs/screenshots/settings.png) |
...
| | |
|---|---|
| ![light](https://raw.githubusercontent.com/TimeCraker/ccweb/main/docs/screenshots/main-en-light.png) | ![en](https://raw.githubusercontent.com/TimeCraker/ccweb/main/docs/screenshots/main-en-dark.png) |
| Light 主题 | English UI |
</details>

## Troubleshooting

- **换工作区后模型仍知道其他项目的内容**:默认档位「全量」会加载 `~/.claude` 全套(全局 CLAUDE.md / rules / skills 描述 / hooks),与 claude CLI 行为一致。要会话隔离,在设置 → 模型 → 上下文隔离切「仅项目」(只加载工作区自身配置)或「纯净」(零注入),新会话生效
