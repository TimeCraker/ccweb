...
const ZH = {
  ...
  'ctx.title': '上下文',
  'ctx.tab.context': '上下文',
  'ctx.tab.trajectory': '调用轨迹',
  'ctx.noTrajectory': '对话后显示调用轨迹',
  ...
  'st.model.scope': '上下文隔离',
  'st.model.scopeNote': '新会话生效',
  'st.model.scope.full': '全量',
  'st.model.scope.project': '仅项目',
  'st.model.scope.none': '纯净',
  'st.model.scopeHelp':
    '全量=~/.claude 全套(全局 CLAUDE.md/规则/Skills/hooks,与 claude CLI 一致);仅项目=只加载工作区自己的配置,会话互不串记忆;纯净=不加载任何文件系统配置',
  ...
  'err.turnError': (code: string) => `本轮出错(${code})`,
  // 工作区
  'ws.title': '启动目录',
  'ws.none': '暂无历史工作区',
...
const EN: Dict = {
  ...
  'cp.imageOnly': '(image)',
  'cp.previewImage': 'View image',
  'ms.running': 'Working…',
  'ms.regenerate': 'Regenerate',
  'ms.fork': 'Fork session',
  'ms.copy': 'Copy',
  'ms.copied': 'Copied',
  ...
  'mt.cost': 'Cost',
  'ctx.title': 'Context',
  'ctx.tab.context': 'Context',
  'ctx.tab.trajectory': 'Trajectory',
  'ctx.noTrajectory': 'Tool trajectory appears after the chat starts',
  ...
  'q.title': 'Answer needed',
  'q.other': 'Other answer',
  'q.submit': 'Submit',
  'q.skip': 'Skip',
  'tl.params': 'Arguments',
  'tl.result': 'Result',
  'tl.diffPreview': 'Change preview',
  'tl.running': 'Running',
  'tl.done': 'Done',
  'tl.error': 'Failed',
  'tl.streaming': 'Args',
  ...
  'st.model.effortNote': 'new sessions',
  'st.model.scope': 'Context isolation',
  'st.model.scopeNote': 'new sessions',
  'st.model.scope.full': 'Full',
  'st.model.scope.project': 'Project only',
  'st.model.scope.none': 'Clean',
  'st.model.scopeHelp':
    'Full = entire ~/.claude (global CLAUDE.md / rules / skills / hooks, same as claude CLI); Project only = workspace-level config only, no cross-workspace memory; Clean = no filesystem settings loaded',
  ...
  'err.turnError': (code: string) => `Turn failed (${code})`,
